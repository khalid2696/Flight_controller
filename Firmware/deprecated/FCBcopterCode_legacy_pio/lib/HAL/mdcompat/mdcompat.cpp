...
I2C i2c_3(I2C3_SDA_PIN, I2C3_SCL_PIN);
Ticker stamper;
...
int mbed_i2c_write(
    ...
    i2c_3.write((int)slave_addr<<1,x,length+1,0);//sending register adress first to indicate which register we are writing and a write register(0)
    return 0;
}
...
int mbed_i2c_read(
    ...
    i2c_3.write((int)slave_addr<<1,RA, 1, 1); // no stop
    i2c_3.read((int)slave_addr<<1,(char*)data, length, 0);
    return 0;
}
