...
struct i2c_integral_frame {
 ...
 I2C i2c_flow(I2C3_SDA_PIN, I2C3_SCL_PIN);
...
int px4flow::update_flow(float gx_flow_frame, float gy_flow_frame)//angular velocity given in flow sensor frame
{
  ...
  i2c_flow.read(ADDR_PX4FLOW<<1 , frame_temp, 22, 0);
  frame_update();
  i2c_flow.write(ADDR_PX4FLOW<<1, frame_int_register, 1);
  i2c_flow.read(ADDR_PX4FLOW<<1 , frame_int_temp, 26, 0);// have to read 26 bytes instead of 25
...
int px4flow::initialize()
{
  ...
  i2c_flow.write(ADDR_PX4FLOW<<1, frame_int_register, 1);
  return i2c_flow.read(ADDR_PX4FLOW<<1 , frame_int_temp, 26, 0);// 0 means success
}
