...
******************************************************************************/
#define USE_WIKIPEDIA_EULER_COMPUTE //IMPORTANT
#define USE_NEW_Q_TO_FLOAT //IMPORTANT
...
#ifndef USE_NEW_Q_TO_FLOAT
//sparkfun library seems to be shitty. https://github.com/sparkfun/SparkFun_MPU-9250-DMP_Arduino_Library/issues/5
float MPU9250_DMP::qToFloat(long number, unsigned char q)
{
	//This code is doing perfect bullshit. results in jumpy incorrect quaternion which messes up everything else
	unsigned long mask;
	for (int i=0; i<q; i++)
	{
		mask |= (1<<i);
...
#endif
#ifdef USE_NEW_Q_TO_FLOAT
//Doing all crazy stuff to get this stupid imu working
float MPU9250_DMP::qToFloat(long number, unsigned char q)
{
	 return (float)((double)number / (double)(1 << q));
}
#endif
#ifndef USE_WIKIPEDIA_EULER_COMPUTE
...
#ifdef USE_WIKIPEDIA_EULER_COMPUTE
 void MPU9250_DMP::computeEulerAngles( bool degrees)
{
	...
	The stock code given in this very library for conversion is incorrect. Both the signs of angles as
	well as the angles seem to be incorrect.
	*/
	float dqw = qToFloat(qw, 30);
	float dqx = qToFloat(qx, 30);
	float dqy = qToFloat(qy, 30);
